...
eur = {}
crypto_value = {}
CRYPTO_LIST = ["bitcoin", "ethereum", "tether", "dogecoin", "litecoin"]
CRYPTO_LOGO = {
    "bitcoin": "https://cdn.discordapp.com/attachments/862133730226995210/862134772738424832/2000px-BTC_Logo.svg.png",
    "ethereum": "https://cdn.discordapp.com/attachments/862133730226995210/862154715348205588/21-218069_blockchains-"
                "contracts-classic-blockchain-organisations-ethereum-logo-png.png",
    "tether": "https://cdn.discordapp.com/attachments/862133730226995210/862155797235761202/tether-usdt-logo.png",
    "dogecoin": "https://cdn.discordapp.com/attachments/862133730226995210/862154713989906463/dogecoin-logo-vector.png",
    "litecoin": "https://cdn.discordapp.com/attachments/862133730226995210/862155332720787466/download.png"}
CHANGE_TIMES = ["1h", "24h", "7d"]
SUPPORTED_CURRENCY = ["USD", "GBP", "EUR"]
    ...
    if last_updated + datetime.timedelta(hours=1) <= datetime.datetime.now() :
        ...
        for currency in SUPPORTED_CURRENCY:
            response = requests.get(
                f'https://api.coingecko.com/api/v3/coins/markets?vs_currency={currency.lower()}&ids=ethereum%2Cbitcoin'
                f'%2Clitecoin%2Cdogecoin%2Ctether&order=market_cap_desc&per_page=100&page=1&sparkline=false&price'
                f'_change_percentage=1h%2C24h%2C7d').json()
            i = 0
            for selected_crypto in CRYPTO_LIST:
                if currency == "USD":
                    usd[selected_crypto] = response[i]
                if currency == "GBP":
                    gbp[selected_crypto] = response[i]
                if currency == "EUR":
                    eur[selected_crypto] = response[i]
                i += 1
...
@slash.slash(
    name="cryptos",
    description="Gives the actual value of a choosen cryptocurrency",
    guild_ids=guild_ids,
    options=[
        create_option(
            name="cryptocurrency",
...
async def crypto(ctx, cryptocurrency: str): 
    ...
    global crypto_value
    get_crypto_value()
    embed_var = discord.Embed(title=f"{cryptocurrency.upper()} ACTUAL VALUE",
                              description="Powered by [CoinGecko](https://coingecko.com)",
                              color=0xf79413)
    if cryptocurrency.lower() in CRYPTO_LIST:
        embed_var.set_thumbnail(url=f"{CRYPTO_LOGO[cryptocurrency.lower()]}")
    else:
        ctx.send(r"Sorry ¯\(°_o)/¯ i do not have any information on this cryptocurrency called : " + cryptocurrency)
        return
    ...
    gbp_str = ""
    eur_str = ""
    usd_change_1h = f"**1 Hour** : None"
    gbp_change_1h = f"**1 Hour** : None"
    eur_change_1h = f"**1 Hour** : None"
    usd_change_1d = f"**1 Day** : None"
    gbp_change_1d = f"**1 Day** : None"
    eur_change_1d = f"**1 Day** : None"
    usd_change_7d = f"**7 days** : None"
    gbp_change_7d = f"**7 days** : None"
    eur_change_7d = f"**7 days** : None"
    usd_value = f"**Value** : None"
    gbp_value = f"**Value** : None"
    eur_value = f"**Value** : None"
    for change_time in CHANGE_TIMES:
        for currency in SUPPORTED_CURRENCY:
            if crypto_value[currency][f'{cryptocurrency}'][f'price_change_percentage_{change_time}_in_currency'] > 0:
                emoji = f"{up_arrow}"
                sign = "+"
            else:
                emoji = f"{down_arrow}"
                sign = ""
            usd_str = f"{emoji} {sign}{round(crypto_value[currency][f'{cryptocurrency}'][f'price_change_percentage_{change_time}_in_currency'],2):,}%\n" if currency == "USD" else usd_str
            gbp_str = f"{emoji} {sign}{round(crypto_value[currency][f'{cryptocurrency}'][f'price_change_percentage_{change_time}_in_currency'],2):,}%\n" if currency == "GBP" else gbp_str
            eur_str = f"{emoji} {sign}{round(crypto_value[currency][f'{cryptocurrency}'][f'price_change_percentage_{change_time}_in_currency'],2):,}%\n" if currency == "EUR" else eur_str
        if change_time == "1h":
            usd_change_1h = f"**1 Hour** : {usd_str}"
            gbp_change_1h = f"**1 Hour** : {gbp_str}"
            eur_change_1h = f"**1 Hour** : {eur_str}"
        elif change_time == "24h":
            usd_change_1d = f"**1 Day** :  {usd_str}"
            gbp_change_1d = f"**1 Day** :  {gbp_str}"
            eur_change_1d = f"**1 Day** :  {eur_str}"
        elif change_time == "7d":
            usd_change_7d = f"**7 days** : {usd_str}"
            gbp_change_7d = f"**7 days** : {gbp_str}"
            eur_change_7d = f"**7 days** : {eur_str}"
    ...
    embed_var.add_field(name=f"**USD**",
                        value=f"{usd_value}{usd_change_1h}{usd_change_1d}{usd_change_7d}",
                        inline=False)
    embed_var.add_field(name=f"**GBP**",
                        value=f"{gbp_value}{gbp_change_1h}{gbp_change_1d}{gbp_change_7d}",
                        inline=False)
    embed_var.add_field(name=f"**EUR**",
                        value=f"{eur_value}{eur_change_1h}{eur_change_1d}{eur_change_7d}",
                        inline=False)
...
async def post_button(ctx):
    embed = discord.Embed(title="✧════════•**MEMBERSHIP ACCESS**•══════✧",
                          description="**► TO GAIN MEMBERSHIP ACCESS PLEASE CLICK\n"
                                      "ON THE FOLLOWING BUTTON** <a:check_ravena:853332159564349491>",
                          color=0x2f3136)
    await ctx.send(embed=embed,
                   components=[Button(style=ButtonStyle.blue, label="I have read the rules and agree to respect them")])
    await ctx.message.delete()
...
@slash.slash(
    name="ppic",
    description="Returns someone's profile picture if no argument given, returns command caller's one",
    options=[
        create_option(
            name="member",
...
async def anime(ctx, anime_name):
    await ctx.defer()
    try:
        search = mal.AnimeSearch(f"{anime_name}")
        result = search.results[0]
        result = mal.Anime(result.mal_id)
        embed_var = discord.Embed(
            title=f"{result.title.upper()}",
            description=f"**[INFO]({result.url})** TAKEN FROM **[MYANIMELIST](https://myanimelist.net/)**",
            color=0xff7575)
        ...
        await ctx.send(f'Sorry im kinda dumb so i did not find any result for " {anime_name} "')
...
async def wisdom(ctx,display: str, question: str):
    wisdom_str = f"{ctx.author.name} was too shy to show to" \
             f" everyone his/her question" if display == "n" else f"{ctx.author.name} asked: {question.capitalize()}"
    embed_var = discord.Embed(
        title=f"A PIECE OF WISDOM",
        description=f"{wisdom_str}",
        color=0x36393f)
    answer = randint(0, 1)
    if answer == 0:
        answer = "Yes"
...
async def slap(ctx, member: discord.Member):
    ...
    if member.name == ctx.author.name:
        ...
        description = ctx.author.name + r" you mother fuc...\*dies\*"
    embed_var = discord.Embed(
        title=f"**{ctx.author.name}** shots **{member.name}**",
        ...
        for line in text:
            if not line == "\n" or line == " ":
                ...
                if current_line_index == rand_nbr:
                    random_line = line
    embed_var = discord.Embed(
        title=f"**{ctx.author.name}** requested a quote !",
        description=f"```{random_line}```",
        color=0x36393f)
    await ctx.send(embed=embed_var)
...
async def flipcoin(ctx):
    answ = "HEADS" if randint(0, 1) == 0 else "TAILS"
    embed_var = discord.Embed(
        title=f"**{ctx.author.name}** flipped a coin ! 🍀",
        description=f"Guess what ? It landed on **{answ}** !",
        color=0x36393f)
    await ctx.send(embed=embed_var)
...
async def rolldice(ctx, dice_count=1, face_count=6):
    ...
    if dice_count == 1:
        embed_var = discord.Embed(
            title=f"**{ctx.author.name.upper()}** ROLLED A {face_count} FACED DICE ! 🎲",
            description=f"IT LANDED ON **{answ[0]}** !",
            color=0xff915e)
    else:
        embed_var = discord.Embed(
            title=f"**{ctx.author.name.upper()}** ROLLED {dice_count} DICES WITH {face_count} FACES EACH ! 🎲",
            description=f"",
            color=0xff915e)
        e = 1
        for ans in answ:
            embed_var.add_field(
                name=f"Dice number {e}",
                value=f"Landed on **{answ[e-1]}** !")
            e += 1
    await ctx.send(embed=embed_var)


@slash.slash(name="news", description="Returns news on a given topic", guild_ids=guild_ids)
async def get_news(ctx, *, subjects):
    await ctx.defer()
    try:
        news_link = await news_api.getnews(subjects)
        await ctx.send('Here is the news : ' + news_link)
    except IndexError:
...
async def daikokuten_help(ctx):
    embed = discord.Embed(
        title="✧═════•**COMMANDS LIST**•═════✧",
        description="If you want to **interact** with the **bot**, here are the **commands**.\n You must **keep** the "
                    "**correct syntax** of the **command** for it to **work properly**",
        color=0x2f3136)
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/"
                            "848876190596071434/853409759918948382/6243-blurple-slashcommands.png")
    embed.add_field(
        name="__**NEWS COMMAND**__",
        value="*Return news on a given topic*\n**Usage** :\n ```/news topic```",
        inline=False)
    embed.add_field(
        name="__**ANIME COMMAND**__",
        value="*Returns info on a given anime*\n**Usage** :\n ```/anime animeName```",
        inline=False)
    embed.add_field(
        name="__**SHOT COMMAND**__",
        value="*Shots someone*\n**Usage** :\n ```/shot @mention```",
        inline=False)
    embed.add_field(
        name="__**KISS COMMAND**__",
        value="*Kisses someone*\n**Usage** :\n ```/kiss @mention```",
        inline=False)
    embed.add_field(
        name="__**HUG COMMAND**__",
        value="*Hugs someone*\n**Usage** :\n ```/hug @mention```",
        inline=False)
    embed.add_field(
        name="__**SLAP COMMAND**__",
        value="*Slaps someone*\n**Usage** :\n ```/slap @mention```",
        inline=False)
    embed.add_field(
        name="__**WISDOM COMMAND**__",
        value="*Gives wisdom*\n**Usage** :\n ```/wisdom show/hide question```",
        inline=False)
    embed.add_field(
        name="__**CRYPTO COMMAND**__",
        value="*Gives a given cryptocurrency's actual value*\n**Usage** :\n ```/crypto cryptocurrency```",
        inline=False)
    embed.add_field(
        name="__**ROLL DICE COMMAND**__",
        value="*Rolls a dice*\n**Usage** :\n ```/rolldice numberOfDice numberOfFacePerDice```",
        inline=False)
    embed.add_field(
        name="__**FLIPCOIN COMMAND**__",
        value="*Flips a coin*\n**Usage** :\n ```/flipcoin```",
        inline=False)
    embed.add_field(
        name="__**QUOTE COMMAND**__",
        value="*Gives out a random quote*\n**Usage** :\n ```/quote```",
        inline=False)
    embed.add_field(
        name="__**HENTAI COMMAND**__",
        value="*Return a NSFW image on a given character or anime*\n**Usage** :\n ```/hentai characterName```\n```"
              "/hentai animeName```\n```/hentai characterName_(animeName)```\n",
        inline=True)
    embed.add_field(
        name="__**PROFILE PIC COMMAND**__",
        value="*Returns someone's profile picture\n or the command caller's one if no user"
              " provided*\n**Usage** :\n ```/ppic @mention```",
        inline=False)
    embed.add_field(
        name="__**DHELP COMMAND**__",
        value="*Shows commands list*\n**Usage** :\n ```/dhelp```",
        inline=False)
    await ctx.send(embed=embed)


@slash.slash(name="hentai", description="Returns hentai on a given topic", guild_ids=guild_ids)
async def get_hentai(ctx, tags):
    if ctx.channel.nsfw:
        tags = tags.split(" ")
        await ctx.defer()
        banned_tags = []
        ...
        if sauce == "None":
            embed_var = discord.Embed(
                title="Unfortunately no NSFW sauce have been found",
                description=f"The following research tags : {', '.join(map(str, tags))}"
                            f" didn't correspond to any any NSFW image from [gelbooru.com](https://gelbooru.com)",
                color=0xff99e0)
            embed_var.add_field(
                name="Requested by",
            ...
            embed_var = discord.Embed(
                title="THE HOLY REQUESTED SAUCE",
...
async def hentai(ctx, *args):
    if ctx.channel.nsfw:
        ...
        if sauce == "None":
            embed_var = discord.Embed(
                title="Unfortunately no NSFW sauce have been found",
                description=f"The following research tags : {', '.join(map(str, args))} "
                            f"didn't correspond to any any NSFW image from [gelbooru.com](https://gelbooru.com)",
                color=0xff99e0)
        ...
        if res.channel.id == 855253520675897344:
            ...
            if get(res.guild.roles,id=816808289512718337) in member.roles:
                await res.respond(
                    ...
                    content=r"¯\_(ツ)_/¯ It looks like you are already a verified member of " +
                            f"{guild.name} <a:check_ravena:853332159564349491>"
                )
            else:
                await member.add_roles(get(res.guild.roles,id=816808289512718337))
                await res.respond(
                    type=InteractionType.ChannelMessageWithSource,
                    content=f"Congratulations <a:zero_two:853224128412123136> you are now a verified member of"
                            f" {guild.name} <a:check_ravena:853332159564349491>"
                )
